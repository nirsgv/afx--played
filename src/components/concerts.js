...
function Concerts({ venues }) {
  const concertsCollection =
    shows && venues.map((item) => getConcertById(item, showsData));
  {
    return concertsCollection &&
      concertsCollection.filter((item) => !!item).length ? (
      <List baseClassName='concerts'>
        {concertsCollection.map((item, index, items) => (
          <Link
            to={`/concert/${item && encodeURIComponent(item.SHOW_ID)}`}
            key={index}
